...
final class PopularMovieListUsecase: MovieListUsecaseType {
    func getMovieList(params: [String : Any]) async throws -> (movies: [MoviesEntity], totalPages: Int) {
        ...
        CoreDataHandler.shared.insert(movieListEntity, type: "Popular")
        return (movieListEntity, response.totalPages ?? 1)
    }
    
    func getStoredMovieList() -> [MoviesEntity] {
        let movies = CoreDataHandler.shared.fetchAllMovies(with: "Popular")
